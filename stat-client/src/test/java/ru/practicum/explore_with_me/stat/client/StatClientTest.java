...
class StatClientTest {
    ...
    @Test
    void getStats_ShouldReturnListOfViewStats_WhenResponseIs200OK() {
        ...
        List<ViewStats> expectedStats = List.of(createViewStats("app1", "/events/1", 10L),
                createViewStats("app2", "/events/2", 5L));
        ...
        List<ViewStats> result = statClient.getStats(start, end, uris, unique);

        // Проверка
        assertNotNull(result);
        assertEquals(2, result.size());
        assertEquals(expectedStats.get(0).getApp(), result.get(0).getApp());
        assertEquals(expectedStats.get(0).getUri(), result.get(0).getUri());
        assertEquals(expectedStats.get(0).getHits(), result.get(0).getHits());
        ...
        assertTrue(capturedUrl.contains("start=" + encodedStart));
        assertTrue(capturedUrl.contains("end=" + encodedEnd));
        assertTrue(capturedUrl.contains("uris=/events/1"));
        assertTrue(capturedUrl.contains("uris=/events/2"));
        assertTrue(capturedUrl.contains("unique=true"));
    }

    @Test
    void getStats_ShouldWorkWithoutUrisAndUnique() {
        LocalDateTime start = LocalDateTime.of(2023, 1, 1, 0, 0, 0);
        LocalDateTime end = LocalDateTime.of(2023, 12, 31, 23, 59, 59);
        ...
        List<ViewStats> result = statClient.getStats(start, end, null, null);
        ...
        assertFalse(capturedUrl.contains("unique="));
    }
